...
static size_t VTL_curl_http_client_WriteCallback(void *contents, size_t size, size_t nmemb, void *userp) {
    ...
    if (!new_body) { free(resp->body); resp->body = NULL; return 0; }
    resp->body = new_body;
...
int VTL_curl_http_client_Request(const char *url, HttpMethod method, const HttpRequest *request, HttpResponse *response) {
    ...
    if (request && request->content_type) {
        ...
        hdrs = curl_slist_append(NULL, ct_header);
    }
    if (hdrs) curl_easy_setopt(curl, CURLOPT_HTTPHEADER, hdrs);
    switch (method) {
        case HTTP_POST:
            curl_easy_setopt(curl, CURLOPT_POST, 1L);
            if (request && request->body) curl_easy_setopt(curl, CURLOPT_POSTFIELDS, request->body);
            break;
        case HTTP_PUT:
        ...
        case HTTP_DELETE:
            ...
            break;
        case HTTP_GET: default: break;
    }
    CURLcode res = curl_easy_perform(curl);
    if (res != CURLE_OK) { if (hdrs) curl_slist_free_all(hdrs); curl_easy_cleanup(curl); return 0; }
...
void VTL_curl_http_client_ResponseCleanup(HttpResponse *response) {
    if (!response) return;
    if (response->body) { free(response->body); response->body = NULL; }
    response->status_code = 0;
}
...
int VTL_curl_http_client_RequestMultipart(
    const char *url, struct curl_httppost *form,
    const struct curl_slist *extra_headers, HttpResponse *response)
{
    if (!url || !response) { if (form) curl_formfree(form); return 0; }
    struct curl_slist *local_headers = NULL;
    if (extra_headers) {
        struct curl_slist *iter = (struct curl_slist *)extra_headers;
        while (iter) { local_headers = curl_slist_append(local_headers, iter->data); iter = iter->next; }
    }
    CURL *curl = curl_easy_init();
    if (!curl) { if (local_headers) curl_slist_free_all(local_headers); curl_formfree(form); return 0; }
    response->body = NULL; response->status_code = 0;
    curl_easy_setopt(curl, CURLOPT_URL, url);
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, VTL_curl_http_client_WriteCallback);
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, response);
    if (local_headers) curl_easy_setopt(curl, CURLOPT_HTTPHEADER, local_headers);
    curl_easy_setopt(curl, CURLOPT_HTTPPOST, form);
    CURLcode res = curl_easy_perform(curl);
    if (res != CURLE_OK) { curl_easy_cleanup(curl); if (local_headers) curl_slist_free_all(local_headers); curl_formfree(form); return 0; }
    curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &response->status_code);
    curl_easy_cleanup(curl);
