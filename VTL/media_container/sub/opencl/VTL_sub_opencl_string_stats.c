...
VTL_AppResult VTL_sub_OpenclStringStats(VTL_OpenCLContext* ctx, const char** in_texts, VTL_StringStats** out_stats, size_t count) {
    cl_int err;
    ...
    cl_mem buf_out_stats = clCreateBuffer(ctx->context, CL_MEM_WRITE_ONLY, count * sizeof(VTL_StringStats), NULL, &err);

    clSetKernelArg(ctx->kernel_string_stats, 0, sizeof(cl_mem), &buf_in_data);
    clSetKernelArg(ctx->kernel_string_stats, 1, sizeof(cl_mem), &buf_offsets);
    clSetKernelArg(ctx->kernel_string_stats, 2, sizeof(cl_mem), &buf_lengths);
    clSetKernelArg(ctx->kernel_string_stats, 3, sizeof(cl_mem), &buf_out_stats);

    size_t global = count;
    err = clEnqueueNDRangeKernel(ctx->queue, ctx->kernel_string_stats, 1, NULL, &global, NULL, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueReadBuffer(ctx->queue, buf_out_stats, CL_TRUE, 0, count * sizeof(VTL_StringStats), stats, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        clReleaseMemObject(buf_in_data);
        clReleaseMemObject(buf_offsets);
        clReleaseMemObject(buf_lengths);
