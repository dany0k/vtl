...
VTL_AppResult VTL_sub_OpenclCleanTags(VTL_OpenCLContext *ctx, const char **in_texts, char **out_texts, size_t count) {
    ...
    cl_mem buf_offsets = clCreateBuffer(ctx->context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, count * sizeof(int),
                                        in_offsets, &err);
    cl_mem buf_lengths = clCreateBuffer(ctx->context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, count * sizeof(int),
                                        in_lengths, &err);
    cl_mem buf_out_data = clCreateBuffer(ctx->context, CL_MEM_WRITE_ONLY, total_out, NULL, &err);
    cl_mem buf_out_offsets = clCreateBuffer(ctx->context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, count * sizeof(int),
                                            out_offsets, &err);

    clSetKernelArg(ctx->kernel_clean_tags, 0, sizeof(cl_mem), &buf_in_data);
    clSetKernelArg(ctx->kernel_clean_tags, 1, sizeof(cl_mem), &buf_offsets);
    clSetKernelArg(ctx->kernel_clean_tags, 2, sizeof(cl_mem), &buf_lengths);
    clSetKernelArg(ctx->kernel_clean_tags, 3, sizeof(cl_mem), &buf_out_data);
    clSetKernelArg(ctx->kernel_clean_tags, 4, sizeof(cl_mem), &buf_out_offsets);

    size_t global = count;
    err = clEnqueueNDRangeKernel(ctx->queue, ctx->kernel_clean_tags, 1, NULL, &global, NULL, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
