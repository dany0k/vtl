...
VTL_AppResult VTL_sub_OpenclFormatNumbers(VTL_OpenCLContext* ctx, const char** in_texts, char*** out_texts, size_t count, char sep, int group_len) {
    cl_int err;
    ...
    clSetKernelArg(ctx->kernel_format_numbers, 0, sizeof(cl_mem), &buf_in_data);
    clSetKernelArg(ctx->kernel_format_numbers, 1, sizeof(cl_mem), &buf_offsets);
    clSetKernelArg(ctx->kernel_format_numbers, 2, sizeof(cl_mem), &buf_lengths);
    clSetKernelArg(ctx->kernel_format_numbers, 3, sizeof(cl_mem), &buf_out_data);
    clSetKernelArg(ctx->kernel_format_numbers, 4, sizeof(cl_mem), &buf_out_offsets);
    clSetKernelArg(ctx->kernel_format_numbers, 5, sizeof(char), &sep);
    clSetKernelArg(ctx->kernel_format_numbers, 6, sizeof(int), &group_len);

    size_t global = count;
    err = clEnqueueNDRangeKernel(ctx->queue, ctx->kernel_format_numbers, 1, NULL, &global, NULL, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
